...
function main() {
  ...
  const bundleBlock = between(mainJs, "async function createSupportBundle", "function revealLastSupportBundle");
  ...
  assertIncludes(mainJs, "zerolag:reveal-support-bundle", "IPC must expose safe support bundle reveal.");
  assertIncludes(mainJs, "shell.showItemInFolder(lastSupportBundlePath)", "Support bundle reveal must stay in the main process.");
  assertOk(!bundleBlock.includes("filePath: result.filePath"), "Support bundle creation must not return the full local file path to the renderer.");
  ...
  assertIncludes(preloadJs, "revealSupportBundle", "Preload must expose safe support bundle reveal.");
  ...
  assertIncludes(indexHtml, 'id="supportRevealButton"', "Toolbox must include a hidden support bundle reveal action.");
  assertIncludes(indexHtml, 'id="supportRevealButton" class="wide" hidden disabled', "Support bundle reveal action must be hidden until a bundle exists.");
  ...
  assertIncludes(rendererJs, "window.zeroLag.revealSupportBundle", "Renderer must reveal support bundles through IPC.");
  assertIncludes(rendererJs, "setSupportBundleRevealReady(result.canReveal)", "Renderer must reveal the file-location action only after a bundle is created.");
  ...
  assertIncludes(rendererJs, "supportRevealButton.addEventListener", "Renderer must wire support bundle reveal action.");
  ...
  assertIncludes(stylesCss, ".tool-actions .wide", "Wide toolbox actions must have layout support.");

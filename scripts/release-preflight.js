...
function main() {
  ...
  addIssue(issues, fileExists("server/payment-provider.js"), "Payment provider adapter module is missing.");
  ...
  addIssue(issues, fileExists("scripts/release-next-step.js"), "Release next-step guidance tool is missing.");
  addIssue(issues, fileExists("scripts/release-next-step-smoke-test.js"), "Release next-step guidance smoke test is missing.");
  addIssue(issues, fileExists("scripts/formal-release-inputs.js"), "Formal release input checklist tool is missing.");
  addIssue(issues, fileExists("scripts/formal-release-inputs-smoke-test.js"), "Formal release input checklist smoke test is missing.");
  ...
  addIssue(issues, (packageJson.scripts.ci || "").includes("npm run payment:provider:smoke"), "npm run ci must verify payment provider adapter coverage.");
  ...
  addIssue(issues, (packageJson.scripts.ci || "").includes("npm run release:status:smoke"), "npm run ci must verify beginner-facing release status reporting.");
  addIssue(issues, (packageJson.scripts.ci || "").includes("npm run release:next:smoke"), "npm run ci must verify beginner-facing next-step release guidance.");
  addIssue(issues, (packageJson.scripts.ci || "").includes("npm run release:inputs:smoke"), "npm run ci must verify formal release input validation.");
  addIssue(issues, (packageJson.scripts.ci || "").includes("npm run release:rehearsal:smoke"), "npm run ci must verify local release rehearsal planning.");
  ...
  addIssue(issues, mainJs.includes("app.requestSingleInstanceLock()"), "Desktop app must prevent multiple normal UI instances from running at once.");
  addIssue(issues, mainJs.includes('app.on("second-instance"'), "Desktop app must reveal the existing window when launched again.");

...
- Harden signed runtime sessions with server-issued session IDs and key rotation. Server-issued runtime session IDs, key-version labels, HMAC proofs, configurable server-side RSA proofs, and runtime guard public-key verification are in place; native wrapper key embedding and full production key rotation are still required.

...
The installer service script points the installed `ZeroLag.exe` at `--runtime-guard-service`, which starts the guard worker without opening the desktop UI. Run `npm run guard:service:smoke` and `npm run guard:runtime:smoke` before packaging to verify the guard assets remain visible, uninstallable, and limited to runtime-state cleanup.
